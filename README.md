...
Необходимые пререквизиты для запуска программы:
* Компас-3D (любая версия);
* Справка SDK для Компас v22 или старше, в web-формате (а не `.chm`), распакованная из zip-архива;
* пакет `pywin32`.
...
Отсутствующие зависимости, включая пакет `pywin32`, установятся автоматически.

# Запуск

Структура рабочей директории программы:
```
./
├── assets/
├── KompasAPI_stubs_generator/
│   ├── utils/
│   │   └── ...
│   ├── __init__.py
│   ├── __main__.py
...
Для запуска программы необходимо находится в рабочей директории. Команда, выполняющая действия по умолчанию:
```
