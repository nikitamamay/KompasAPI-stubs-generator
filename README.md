...
Программа для создания stub-файлов с объявлениями классов, методов, свойств, аннотациями типов и документацией из Справки SDK для Python-модулей API САПР «Компас-3D».

> Также обратите внимание на **[библиотеку KompasAPI-01](https://github.com/nikitamamay/KompasAPI-01)** — это обёртка для API Компас-3D, которая заметно упрощает написание скриптов.
>
> В её основе представлены stub-файлы и некоторые модули, сгенерированные программой `KompasAPI-stubs-generator`.

C помощью `KompasAPI-stubs-generator` удаётся извлечь полезную информацию из 610 MB страниц Справки SDK и уложить её всего в 10 MB.
...
Программа `KompasAPI_stubs_generator` предназначена для создания stub-файлов для модулей Компас API с использованием информации из Справки SDK Компас.
...
Для запуска программы необходимо вызвать команду ниже. Справка SDK Компас должна быть распакована из zip-архива.
```
python -m KompasAPI_stubs_generator -s "/путь/к_папке/Справки_SDK_Компас"
```

При запуске можно выбрать: для какой версии API Компас будет выполнятся обработка; какие этапы программы будут выполняться; в каких папках записывать временные и результирующие файлы и др.
Подробное описание опций командной строки:
```
python -m KompasAPI_stubs_generator --help
```


## Создаваемые файлы

Приложение в процессе работы создает результирующие файлы по умолчанию в рабочей директории `./`, а промежуточные файлы (включая log-файл) — в папке `./auxdir`.
...
В результирующих stub-файлах даются указания о наследовании классов. Наследование классов применено для того, чтобы сократить количество повторяющихся объявлений методов и свойств.
...
Стоит помнить, что функция `isinstance()` будет работать не так, как если бы было действительное наследование классов друг от друга. Поэтому для проверки принадлежности объекта к определенному классу следует учитывать иерархию классов интерфейсов (в модуле `classes_hierarchy.py`). Пример такой реализации функции `isinstance_for_KompasAPI()` есть в библиотеке [KompasAPI-01](https://github.com/nikitamamay/KompasAPI-01).
...
1. `parse_module` выполняет анализ модулей `Kompas6API5` и `KompasAPI7`, сохраняет перечень классов, их свойств и методов.
...
5. `generate_stub` формирует stub-файлы.

6. `generate_hierarchy` создает python-модуль, который хранит информацию о наследовании классов интерфейсов Компас API.

7. `generate_constants` создает python-модуль, в котором указаны константы и перечисления Компас API.
...
**Кодировка.** Кодировка Python-модулей API — это `cp1251`, а не `UTF-8`. Кодировка `cp1251` не всегда определяется автоматически в VSCode (необходимо явно открывать файл модуля API в отдельной вкладке IDE), и это приводит к тому, что невозможно прочитать и без того скудную документацию к методам и классам.
